...
class Client extends Base {
  ...
  sign(stringToSign) {
    const hmacDigest = Base64.stringify(hmacSHA256(stringToSign, this.appSecret));
    return hmacDigest;
  }
  ...
  async request(method, url, opts, originData) {
    ...
    var response = await fetch(format(parsedUrl), {
      method: method,
      headers: headers,
      body: opts.data,
    });
    ...
    if (code !== 200) {
      ...
      var message = headers['x-ca-error-message'][0] || '';
